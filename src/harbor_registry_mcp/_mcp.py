...
async def app_lifespan(_app: FastMCP) -> AsyncIterator[dict[str, Any]]:
    ...
    logger.debug("harbor_registry_mcp: startup")
    try:
        yield {}
    finally:
        global _client
        with _client_lock:
            if _client is not None:
                try:
                    ...
                    pass
                _client = None
        logger.debug("harbor_registry_mcp: shutdown — HTTP session closed")


mcp = FastMCP("harbor_registry_mcp", lifespan=app_lifespan)


def get_client() -> HarborClient:
    """Return a cached :class:`HarborClient` (thread-safe lazy-init).
